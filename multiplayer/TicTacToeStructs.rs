...
pub mod TicTacToeStructs {
    ...
    impl Room {
        ...
        pub fn Move(&mut self, msg: String) {
            ...
            let mut buff = bincode::serialize(&Message::new("EnemyMove".to_string(), msg)).unwrap();
            let mut temp = user
                .getStream()
                .try_clone()
    ...
    impl Rooms {
        ...
        pub fn findRoomWithAddr(&mut self, addr: SocketAddr) -> Result<&Room, &'static str> {
            if let Some(pos) = self
                .rooms
                .iter()
                .position(|room| room.isAddrInRoom(addr) == true)
            {
    ...
    impl TicTacToeBoard {
        ...
        pub fn PlaceNextMarker(&mut self, x: usize, y: usize) -> bool {
            if self.winner == 0
                && self.innerBoardsState[x / 3][y / 3] == 0
                && self.boardState[x][y] == 0
            {
                //place
        ...
        pub fn checkBoard(&self) -> i8 {
            for player in 1..3 {
                for dx in 0..3 {
                    if self.Outerhas(dx, 0, player)
                        && self.Outerhas(dx, 1, player)
                        && self.Outerhas(dx, 2, player)
                    {
                ...
                for dy in 0..3 {
                    if self.Outerhas(0, dy, player)
                        && self.Outerhas(1, dy, player)
                        && self.Outerhas(2, dy, player)
                    {
                        println!("Winner vetrical y:{} ", dy);
                        return player;
                    }
                }
                if self.Outerhas(0, 0, player)
                    && self.Outerhas(1, 1, player)
                    && self.Outerhas(2, 2, player)
                {
                    println!("Winner diagonally t->b");
                    return player;
                }
                if self.Outerhas(2, 0, player)
                    && self.Outerhas(1, 1, player)
                    && self.Outerhas(0, 2, player)
                {
        ...
        pub fn checkInnerBoard(&mut self, x: usize, y: usize, player: i8) {
            for dx in 0..3 {
                if self.Innerhas(x + dx, y, player)
                    && self.Innerhas(x + dx, y + 1, player)
                    && self.Innerhas(x + dx, y + 2, player)
                {
            ...
            for dy in 0..3 {
                if self.Innerhas(x, y + dy, player)
                    && self.Innerhas(x + 1, y + dy, player)
                    && self.Innerhas(x + 2, y + dy, player)
                {
                    println!("vetrical y:{} ", dy + y);
                    self.innerBoardsState[x / 3][y / 3] = player;
                }
            }
            if self.Innerhas(x, y, player)
                && self.Innerhas(x + 1, y + 1, player)
                && self.Innerhas(x + 2, y + 2, player)
            {
                println!("diagonally t->b");
                self.innerBoardsState[x / 3][y / 3] = player;
            }
            if self.Innerhas(x + 2, y, player)
                && self.Innerhas(x + 1, y + 1, player)
                && self.Innerhas(x, y + 2, player)
            {
